  ...
  <div class="abandonStep2">
    ...
    <Card>
      ...
      <div class="center">
        ...
        <Button size="large" type="primary" @click="next" :loading="subLoading"  :disabled="buttonFlag"
          >保存，下一步</Button
        >
      </div>
    </Card>
...
export default {
  ...
  created() {
    ...
    this.uploadData.noticeId = this.getStore("abandonPreId");
  },
  activated() {
    if (this.getStore("preAbandon")) {
      this.removeStore("preAbandon");
  ...
  methods: {
    ...
    getViewFiles() {
      viewFiles({
        noticeId: this.getStore("abandonPreId"),
      }).then((res) => {
        if (res.success) {
          if (res.result) {
