...
export default {
  ...
  created() {
    if (this.getStore("changePreId")) {
      this.getProjects(this.getStore("changePreId"));
    } else {
      this.getProjects(this.$route.query.id);
    }
    if (this.getStore("preChange")) {
      this.removeStore("preChange");
    }
  },
  activated() {
    if (this.getStore("changePreId")) {
  ...
  methods: {
    ...
    getProjects(id) {
      getCanTenderList().then((res) => {
        if (this.getStore('changePreClickFlag')) {
          this.clearForm();
          this.removeStore('changePreClickFlag')
        }
        if (res.success) {
          ...
          viewPreFirst(id).then((data) => {
            if (data.success) {
              ...
              if (this.getStore("changePreId")) {
                ...
                this.formItem.isDelay = this.preInfo.isDelay;
                this.formItem.openBidTime = this.preInfo.openBidTime;
                this.formItem.title = this.preInfo.title;
              } else {
                this.formItem.title = this.preInfo.title + "（变更）";
              }
            }
          });
        }
      ...
      if (this.getStore("changePreId")) {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            savePreChangeEdit(data).then((res) => {
        ...
        this.$refs[name].validate((valid) => {
          if (valid) {
            ...
            this.subLoading = true;
            savePreChange(data).then((res) => {
              this.subLoading = false;
              if (res.success) {
                this.setStore("changePreId", res.result.id);
                this.$emit("update:currentStep", 1);
              }
            });
          }
