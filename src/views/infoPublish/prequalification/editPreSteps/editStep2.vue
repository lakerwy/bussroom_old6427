...
export default {
  ...
  data() {
    return {
      timeOptions: {
        disabledDate(date) {
          return date && date.valueOf() < new Date(window.localStorage.getItem('preObtainFileTimeEdit')).getTime() - 86400000;
        }
      },
      sunLoading: false,
      timeRange: [],
  ...
  methods: {
    // 时间选择
    timeChange(v) {
      this.formItem.postDeadlineStart = v[0];
      this.formItem.postDeadlineEnd = v[1];
    },
    pre() {
      this.$emit("update:currentStep", 0);
    },
