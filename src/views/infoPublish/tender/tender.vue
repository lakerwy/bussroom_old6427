  ...
  <div id="announcement">
    <Card>
      ...
      <Row style="margin-bottom: 10px">
        <Button type="primary" icon="md-add" @click="addTender"
          >新建招标公告</Button
        >
          ...
          <template slot-scope="{ row }" slot="action">
            <div class="flex-row">
              ...
              >
                ...
                <a @click="correctTender(row)" v-show="row.noticeType !== 6">
                  <Icon type="md-swap" />
                  发变更
                </a>
                <a @click="abandonTender(row)" v-show="row.noticeType !== 6">
                  <Icon type="md-stats" />
                  发废标
                </a>
                <a @click="addCandidate(row)" v-show="row.noticeType !== 6">
                  <Icon type="md-grid" />
                  发中标候选人公示
                </a>
                <a @click="addResult(row)" v-show="row.noticeType !== 6">
                  <Icon type="md-hammer" />
                  发中标结果公告
                </a>
              </div>
...
import { getTenderList } from "@/api/publishApi";
export default {
  name: "tender",
  components: {
    commonShare,
  ...
  methods: {
    getDataList() {
      this.loading = true;
      getTenderList(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
    ...
    correctTender(v) {
      if (
        ...
        this.$Modal.confirm({
          title: "提示",
          content: "当前已有变更公告正在被发布，确认要发布新的变更公告？",
          onOk: () => {
            this.setStore('changeTenderClickFlag',true)
    ...
    abandonTender(v) {
      if (
        ...
        this.$Modal.confirm({
          ...
          onOk: () => {
            this.setStore('abandonTenderClickFlag',true)
    ...
    edit(v) {
      if (v.noticeType == 4) {
      ...
      } else if (v.noticeType == 5) {
        if (
          this.$store.state.app.pageOpenedList.some(
            (item) => item.name === "editCorrectTender"
          )
        ) {
          this.$Modal.confirm({
            title: "提示",
            ...
            onOk: () => {
              this.setStore("editCorrectTender", true);
              this.$router.push({
                name: "editCorrectTender",
                query: {
                  id: v.id,
                },
              });
        ...
        if (
          this.$store.state.app.pageOpenedList.some(
            (item) => item.name === "editAbandonTender"
          )
        ) {
          this.$Modal.confirm({
            title: "提示",
            content: "当前已有废标公告正在被编辑，确认要编辑新废标公告？",
            onOk: () => {
              this.setStore("editAbandonTender", true);
              this.$router.push({
                name: "editAbandonTender",
                query: {
                  id: v.id,
                },
              });
