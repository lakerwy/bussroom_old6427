      ...
      <Card style="padding-bottom: 200px;padding-top:20px;height:100%;overflow-y: auto;" class="inner-card">
        ...
        <Row>
          ...
          <div class="infoText">
            ...
            <p v-if="notice.isDelay == 1">延期开标时间：{{format(new Date(notice.openBidTime), 'yyyy年MM月dd日 HH时mm分ss秒')}}</p>
          </div>
        </Row>
...
<script>
import { saveBuildNotice,tenderPreView } from "@/api/publishApi";
export default {
  name: "correctStep3",
  props: {
    currentStep: {
  ...
  methods: {
    // 预览
    view() {
      tenderPreView(this.getStore("changeTenderId")).then(res => {
        if(res.success) {
