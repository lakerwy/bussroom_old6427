...
export default {
  ...
  activated() {
    if (this.getStore("tenderChange")) {
      this.removeStore("tenderChange");
      this.$emit("update:currentStep", 0);
    }
  },
  methods: {
    index() {
      this.$store.commit("removeTag", "correctTenderStep");
      this.$store.commit("closePage", "correctTenderStep");
      if (
    ...
    next() {
      let id = this.getStore("changeTenderId");
